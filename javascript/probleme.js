document.addEventListener('DOMContentLoaded', () => {
    ...
    if (nextBtn) {
        nextBtn.addEventListener('click', () => {
            console.log('Next button clicked, before=', currentPuzzleIndex);
            currentPuzzleIndex = (currentPuzzleIndex + 1) % puzzles.length;
            console.log('Next button new index=', currentPuzzleIndex);
            const p = puzzles[currentPuzzleIndex];
            if (window.loadPuzzle) window.loadPuzzle(p.fen, p.turn);
            if (instr) instr.textContent = `${p.name} — ${p.turn === 'white' ? 'Blancs' : 'Noirs'} jouent`;
            if (hintEl) hintEl.textContent = '';
        });
    }
});
