...
function updateUserInfo(data) {
    try {
        ...
        if (avatarEl) {
            avatarEl.innerHTML = '';
            const sym = data.user.avatar || '♔';
            if (window.renderSymbol) avatarEl.appendChild(window.renderSymbol(sym));
            else avatarEl.textContent = sym;
        }
...
function updateBadges(data) {
    try {
        ...
        badges.forEach(badge => {
            ...
            badgeEl.innerHTML = '';
            if (window.renderSymbol) {
                const iconNode = window.renderSymbol(badge.icon || '');
                badgeEl.appendChild(iconNode);
...
function updateRecentActivity(data) {
    try {
        ...
        activities.forEach(activity => {
            ...
            if (window.renderSymbol) {
                iconSpan.appendChild(window.renderSymbol(activity.icon || '📌'));
            } else {
        ...
        <div style="background: white; padding: 40px; border-radius: 20px; max-width: 500px; width: 90%; box-shadow: 0 20px 60px rgba(0,0,0,0.3);font-family: Arial, sans-serif;">
            ...
            <div style="margin-bottom: 20px;">
                ...
                <input type="text" id="edit-avatar" value="${data.user.avatar || '♔'}" maxlength="2"
                    style="width: 100%; padding: 12px; border: 2px solid #ddd; border-radius: 8px; font-size: 1em; box-sizing: border-box;">
                <small style="color: #666;">Suggestions: <img class="piece-img" src="https://upload.wikimedia.org/wikipedia/commons/4/42/Chess_klt45.svg" alt="♔"> <img class="piece-img" src="https://upload.wikimedia.org/wikipedia/commons/1/15/Chess_qlt45.svg" alt="♕"> <img class="piece-img" src="https://upload.wikimedia.org/wikipedia/commons/7/72/Chess_rlt45.svg" alt="♖"> <img class="piece-img" src="https://upload.wikimedia.org/wikipedia/commons/b/b1/Chess_blt45.svg" alt="♗"> <img class="piece-img" src="https://upload.wikimedia.org/wikipedia/commons/7/70/Chess_nlt45.svg" alt="♘"> <img class="piece-img" src="https://upload.wikimedia.org/wikipedia/commons/4/45/Chess_plt45.svg" alt="♙"> 👤 🎯 ⚡ 🏆</small>
            </div>
