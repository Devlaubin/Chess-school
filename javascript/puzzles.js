...
function displayPosition(fen) {
    ...
    // helper wrapper in case piece-util not yet loaded
    const makePiece = symbol => {
        if (window.createPieceElement) return window.createPieceElement(symbol);
    ...
    for (let row = 7; row >= 0; row--) {
        for (let col = 0; col < 8; col++) {
            ...
            if (row === 0 && col === 4) square.appendChild(makePiece('♔'));
            if (row === 7 && col === 4) square.appendChild(makePiece('♚'));
